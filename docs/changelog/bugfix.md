...
## 2026-09-05：设备监听与播放时序

| 现象 | 原因 | 调整 |
| --- | --- | --- |
| AI玩具新会话接口 `not found` | 旧 Node 桥接进程仍运行旧路由 | 重启桥接使新增接口生效，补充部署排查 |
| 串口恢复即报告重启成功 | 把串口健康状态当成启动完成 | 改为复位前监听启动标记及随后的 Application idle，支持重新枚举 |
| Speaker 漏判 `onLiveTtsEnd==>false` | 只匹配字面 `$stopRecord` | 兼容实际 false 格式并保留原格式 |
| 配置最大等待 60 秒却 8 秒退出 | 未出现 VAD 开始即结束监听 | 有完播匹配器时持续至 `maxWaitMs` |
| 连续收音窗口被等待消耗 | 明确完播后仍等待录音保护及下一轮唤醒冷却 | 完播结果释放录音保护，复用会话时跳过冷却及固定播放前等待 |
| 完播缺失直接停止下一条 | 缺少重新唤醒恢复分支 | 记录失败，重试当前用例，连续唤醒失败进入启动确认后的重启恢复 |

以上为已落地代码与回归场景；真实硬件整链路和群消息送达仍需现场验收。
