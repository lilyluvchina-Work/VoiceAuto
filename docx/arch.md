...
- 增加前端任务归档能力：日志导入任务已支持本地归档（状态、摘要、结果）。
- 增加结构化报告导出：测试报告新增 JSON / CSV 导出能力，便于外部系统消费。
- 增加大日志降载策略：导入超大日志时自动仅分析最近 N 行并给出提示，降低浏览器卡顿风险。
- 补充生产代理示例：新增 `deploy/nginx/langfuse-proxy.conf.example` 作为部署参考。
